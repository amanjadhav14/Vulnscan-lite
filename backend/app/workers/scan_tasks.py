...
def run_scan(url):
    ...
    try:
        # Standard passive request handshake
        response = requests.get(url, timeout=10, headers={"User-Agent": "VulnScanLite/1.0"})
    ...
    # Target Security Headers Matrix
    security_headers = {
    ...
    for header_name, meta in security_headers.items():
        if header_name in headers:
            ...
            total_score -= 20  # Deduct points for missing elements
            # Dynamic injection into the remediation recommendations module
    ...
    # ─── EXTRA PASSIVE TELEMETRY LOGS ───
    # (Kept intact for UI stability maps)
    domain = url.replace("https://", "").replace("http://", "").split("/")[0]
    
    ssl_data = {"ssl_valid": False, "days_left": 0}
    try:
        ...
        with socket.create_connection((domain, 443), timeout=4) as sock:
            with context.wrap_socket(sock, server_hostname=domain) as ssock:
                ...
                ssl_data = {"ssl_valid": days_left > 0, "days_left": max(0, days_left)}
    except:
        pass
    ...
    return {
        ...
        "remediation": remediation_array,  # Populated array returned to frontend
        "ssl": ssl_data,
        "cms": {
        ...
        "technologies": ["Nginx", "PHP", "OpenSSL"],
        "dns_records": {"A": ["192.168.1.1"], "MX": ["mail.target.local"]},
        "whois": {"registrar": "MarkMonitor Inc.", "organization": "Privacy Protection"}
    }
